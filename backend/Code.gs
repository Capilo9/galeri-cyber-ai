...
function thumbnail_(value){if(typeof value!=='string'||(value.match(/https?:\/\//gi)||[]).length!==1)return '';var u=https_(value);if(!u||/https?:\/\//i.test(u.path))return '';var id=driveId_(value);if(/^lh[3-6]\.googleusercontent\.com$/.test(u.host)){var m=u.path.match(/^\/d\/([\w-]{10,})(?:=|[/?#]|$)/);id=m?m[1]:'';}if(id)return 'https://drive.google.com/thumbnail?id='+id+'&sz=w1200';if(u.host==='drive.google.com'||u.host==='docs.google.com'||u.host==='freeimage.host'&&/^\/i\//.test(u.path))return '';return u.url;}
